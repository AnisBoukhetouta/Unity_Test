...
namespace MalbersAnimations.Controller
{
    ...
    public abstract class MDamager : MonoBehaviourPun, IMDamager, IInteractor
    {
        ...
        {
            if (damagee != null && owner.GetComponent<PhotonView>().IsMine)
            {
                ...
                if (!stat.IsNull)
                {
                    ...
                    //<after>
                    // Get the PhotonView of the damagee
                    /*PhotonView damageeView = damagee.Damagee.GetComponent<PhotonView>();
                    if (damageeView != null)
                    {
                        // Call the RPC method
                        CallApplyDamage(criticalStat.Value, damageeView.ViewID);
                    }*/
        ...
        /*[PunRPC]
        public void ApplyDamage(float damageValue, int targetViewID)
        {
            // Get the PhotonView for the target object
            PhotonView targetView = PhotonView.Find(targetViewID);
            if (targetView != null)
            {
                // Get the target object's IMDamage component
                IMDamage target = targetView.GetComponent<IMDamage>();
                if (target != null)
                {
                    // Create a new StatModifier with the received damage value
                    StatModifier damage = new StatModifier();
                    damage.MinValue = new FloatReference(damageValue);
                    damage.MaxValue = new FloatReference(damageValue);

                    // Call the ReceiveDamage method on the target object
                    target.ReceiveDamage(Direction, HitPosition, Owner, damage, IsCritical, react.Value, CustomReaction, pureDamage.Value, element);
                }
            }
        }

        public void CallApplyDamage(float damageValue, int targetViewID)
        {
            photonView.RPC("ApplyDamage", RpcTarget.All, damageValue, targetViewID);
        }*/
