...
namespace MalbersAnimations.Weapons
{
    ...
    public class MShootable : MWeapon, IShootableWeapon, IThrower
    {
        ...
        public virtual void EquipProjectile()
        {
            ...
            if (ProjectileInstance == null) //Means there's no projectile equipped!
            {
                ...
                if (Projectile.IsPrefab())
                {
                    ...
                    //object[] myCustomInitData = new object[] { GetComponent<PhotonView>().ViewID };
                    //ProjectileInstance = PhotonNetwork.Instantiate(Projectile.name, Pos, Rot, 0, myCustomInitData);
        ...
        public virtual void ReleaseProjectile()
        {
            ...
            {
                GetComponent<PhotonView>().RPC("SyncFireProjectile", RpcTarget.Others);
            }
        ...
        [PunRPC]
        public void SyncFireProjectile()
        {
            // Perform the necessary actions to synchronize the firing of the weapon on other clients
            // For example, you can call the FireProjectile method here
            ReleaseProjectile();
        }
