    ...
    public partial class MWeaponManager
    {
        ...
        public virtual void Holster_Equip(int HolsterID)
        {
            if (UseHolsters && Active && !Paused)
            {
                ...
                {
                    GetComponent<PhotonView>().RPC("SyncWeapon", RpcTarget.Others, HolsterID);
                }
            }
        }
        ...
        public virtual void Holster_SetWeapon(GameObject WeaponGO)
        {
            ...
            if (GetComponent<PhotonView>().IsMine)
            {
                // Get the PhotonView component of the weapon
                PhotonView weaponPhotonView = WeaponGO.GetComponent<PhotonView>();

                if (weaponPhotonView != null)
                {
                    // Sync the weapon assignment on other clients

                    // Transfer the ownership of the weapon's PhotonView to the local player
                    weaponPhotonView.TransferOwnership(PhotonNetwork.LocalPlayer);

                    GetComponent<PhotonView>().RPC("SyncHolsterSetWeapon", RpcTarget.Others, weaponPhotonView.ViewID);
                }
            }

            // Set the weapon on the holster
